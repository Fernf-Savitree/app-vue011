<template>
      ...
      <tbody>
        <tr v-for="product in products" :key="product.product_id">
          ...
          <td>
            <img
              v-if="product.image"
              :src="'http://localhost/APP-VUE011/php_api/image/' + product.image"
              width="100"
            />
          </td>
          <td>
  ...
  <div v-if="isEditMode && editForm.image">
    ...
    <img
      :src="'http://localhost/APP-VUE011/php_api/image/' + editForm.image"
      width="100"
    />
  </div>
</div>
...
export default {
  ...
  setup() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch("http://localhost/app-vue011/php_api/api_product.php");
  ...
  try {
    const res = await fetch(
      "http://localhost/app-vue011/php_api/api_product.php?type=categories"
    );
